...
var config = {
  apiOrigins: {
    ...
    production: 'https://aqueous-spire-71565.herokuapp.com'
  }
};
...
var getOneLocation = function getOneLocation(id) {
  return $.ajax({
...
var addActivity = function addActivity(name) {
  return $.ajax({
...
var addLandmark = function addLandmark(name) {
  return $.ajax({
    ...
    data: { landmark: { name: name } }
  });
};

var addRestaurant = function addRestaurant(name) {
  return $.ajax({
...
var addComment = function addComment(name) {
  return $.ajax({
...
var removeActivity = function removeActivity(name) {
  return $.ajax({
    url: config.apiOrigin + '/activities/' + store.location.id,
    method: 'DELETE',
    headers: {
...
var removeLandmark = function removeLandmark(name) {
  return $.ajax({
    url: config.apiOrigin + '/landmarks/' + store.location.id,
    method: 'DELETE',
    headers: {
...
var removeRestaurant = function removeRestaurant(name) {
  return $.ajax({
    url: config.apiOrigin + '/food/' + store.location.id,
    method: 'DELETE',
    headers: {
...
var removeComment = function removeComment(name) {
  return $.ajax({
    url: config.apiOrigin + '/comments/' + store.location.id,
    method: 'DELETE',
    headers: {
...
var googleMapsLoader = __webpack_require__(20);
var coords = __webpack_require__(21);

// Load templates into variables
var locationsTemplate = __webpack_require__(22);
var activitiesTemplate = __webpack_require__(39);
var landmarksTemplate = __webpack_require__(40);
var restaurantsTemplate = __webpack_require__(41);
var commentsTemplate = __webpack_require__(42);

// Select the display elements for later use
var locationTitleElement = $('#location-title');
var locationsElement = $('#locations-dropdown');
var locationDisplayElement = $('#location-display');
var activitiesElement = $('#activities-display');
var landmarksElement = $('#landmarks-display');
var restaurantsElement = $('#restaurants-display');
var commentsElement = $('#comments-display');

var hideLocationDisplay = function hideLocationDisplay() {
  locationDisplayElement.addClass('hidden');
};

var showLocationDisplay = function showLocationDisplay() {
...
googleMapsLoader.load(function (api) {
  google = api;
...
var getLocationsSuccess = function getLocationsSuccess(response) {
...
var centerMap = function centerMap() {
  ...
  var index = coords.findIndex(function (loc) {
    return loc.name === store.location.name;
  });
  if (index > -1) {
...
var getOneLocationSuccess = function getOneLocationSuccess(response) {
  storeOneLocation(response);
  locationSelected();
};

var addLocationSuccess = function addLocationSuccess(response) {
  storeOneLocation(response);
  if (store.locations) {
...
var removeLocationSuccess = function removeLocationSuccess(response) {
  store.locations = store.locations.filter(function (loc) {
...
var storeLocations = function storeLocations(response) {
  store.locations = response.locations.map(function (loc) {
    return {
      name: loc.name,
      id: loc.id
    };
...
var updateLocations = function updateLocations() {
  if (store.locations.length > 0) {
    locationsElement.html('');
    var newLocations = locationsTemplate({ locations: store.locations });
    locationsElement.append(newLocations);
    $('.location-link').on('click', function (event) {
      api.getOneLocation(event.target.dataset.id).then(getOneLocationSuccess).catch(failure);
    });
  } else {
...
var storeOneLocation = function storeOneLocation(response) {
  store.location = response.location;
};

// Update activities display with the activities in the store
var updateActivityDisplay = function updateActivityDisplay() {
  var newActivities = activitiesTemplate({ activities: store.location.activities });
  activitiesElement.html('');
  activitiesElement.append(newActivities);
  $('.removeactivity-button').on('click', function (event) {
    var name = $(event.delegateTarget).parent().data('name');
    api.removeActivity(name).then(removeActivitySuccess).then(function () {
      return removeActivityFromStore(name);
    }).catch(failure);
  });
};

// landmarks Display
var updateLandmarksDisplay = function updateLandmarksDisplay() {
  var newLandmarks = landmarksTemplate({ landmarks: store.location.landmarks });
  landmarksElement.html('');
  landmarksElement.append(newLandmarks);
  $('.removelandmark-button').on('click', function (event) {
    var name = $(event.delegateTarget).parent().data('name');
    api.removeLandmark(name).then(removeLandmarkSuccess).then(function () {
      return removeLandmarkFromStore(name);
    }).catch(failure);
  });
};

// restaurants Display
var updateRestaurantsDisplay = function updateRestaurantsDisplay() {
  var newRestaurants = restaurantsTemplate({ restaurants: store.location.food });
  restaurantsElement.html('');
  restaurantsElement.append(newRestaurants);
  $('.removerestaurant-button').on('click', function (event) {
    var name = $(event.delegateTarget).parent().data('name');
    api.removeRestaurant(name).then(removeRestaurantSuccess).then(function () {
      return removeRestaurantFromStore(name);
    }).catch(failure);
  });
};

// comments Display
var updateCommentsDisplay = function updateCommentsDisplay() {
  var newComments = commentsTemplate({ comments: store.location.comments });
  commentsElement.html('');
  commentsElement.append(newComments);
  $('.removecomment-button').on('click', function (event) {
    var name = $(event.delegateTarget).parent().data('name');
    api.removeComment(name).then(removeCommentSuccess).then(function () {
      return removeCommentFromStore(name);
    }).catch(failure);
  });
};

// Update display of location with the information in the store
var updateLocationDisplay = function updateLocationDisplay() {
...
var addActivitySuccess = function addActivitySuccess(response) {
  $('#-addactivity-modal').modal('hide');
};

var removeActivitySuccess = function removeActivitySuccess(response) {};

var removeActivityFromStore = function removeActivityFromStore(name) {
  var index = store.location.activities.indexOf(name.toString());
  store.location.activities.splice(index, 1);
  updateActivityDisplay();
};

var removeLandmarkSuccess = function removeLandmarkSuccess(response) {};

var removeLandmarkFromStore = function removeLandmarkFromStore(name) {
  var index = store.location.landmarks.indexOf(name.toString());
  store.location.landmarks.splice(index, 1);
  updateLandmarksDisplay();
};

var removeRestaurantSuccess = function removeRestaurantSuccess(response) {};

var removeRestaurantFromStore = function removeRestaurantFromStore(name) {
  var index = store.location.food.indexOf(name.toString());
...
var removeCommentFromStore = function removeCommentFromStore(name) {
  var index = store.location.comments.indexOf(name.toString());
  store.location.comments.splice(index, 1);
  updateCommentsDisplay();
};

// Add activity to local store
var storeOneActivity = function storeOneActivity(name) {
  store.location.activities.push(name);
  updateActivityDisplay();
};

// Success message for new landmark
var addLandmarkSuccess = function addLandmarkSuccess(response) {
  $('#-addlandmark-modal').modal('hide');
};

// Add landmark to local store
var storeOneLandmark = function storeOneLandmark(name) {
  store.location.landmarks.push(name);
  updateLandmarksDisplay();
};

// Success message for new restaurant
var addRestaurantSuccess = function addRestaurantSuccess(response) {
  $('#-addrestaurant-modal').modal('hide');
};

// Add restaurant to local store
var storeOneRestaurant = function storeOneRestaurant(name) {
  store.location.food.push(name);
  updateRestaurantsDisplay();
};

// Success message for new comment
var addCommentSuccess = function addCommentSuccess(response) {
  $('#-addcomment-modal').modal('hide');
};

// Add comment to local store
var storeOneComment = function storeOneComment(name) {
...
module.exports = {
  getLocationsSuccess: getLocationsSuccess,
  addLocationSuccess: addLocationSuccess,
  removeLocationSuccess: removeLocationSuccess,
  getOneLocationSuccess: getOneLocationSuccess,
  addActivitySuccess: addActivitySuccess,
  storeOneActivity: storeOneActivity,
  addLandmarkSuccess: addLandmarkSuccess,
  storeOneLandmark: storeOneLandmark,
  addRestaurantSuccess: addRestaurantSuccess,
  storeOneRestaurant: storeOneRestaurant,
  addCommentSuccess: addCommentSuccess,
  storeOneComment: storeOneComment,
  failure: failure
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var locEvents = __webpack_require__(43);

$(function () {
  setAPIOrigin(location, config);
  authEvents.addHandlers();
  locEvents.addHandlers();
});
...
var locationsApi = __webpack_require__(6);
var ui = __webpack_require__(19);
var locationsUi = __webpack_require__(10);
...
var onSignIn = function onSignIn(event) {
  ...
  }).then(locationsUi.getLocationsSuccess).catch(ui.signInError);
};
...
var forceSignIn = function forceSignIn() {
  if (!store.user) {
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "<li>\n  <a href=\"#\" class=\"location-link\" data-id=\""
  ...
  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.locations : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "<p><div class=\"list-item\" data-name=\""
    + alias2(alias1(depth0, depth0))
    + "\">\n  <div>\n    "
    + alias2(alias1(depth0, depth0))
    + "\n  </div>\n  <button type=\"button\" class=\"removeactivity-button btn-xs btn-danger\">X</button>\n</div></p>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    var stack1;

  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.activities : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "");
},"useData":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "<p><div class=\"list-item\" data-name=\""
    ...
    + "\">\n  <div>\n    "
    + alias2(alias1(depth0, depth0))
    + "\n  </div>\n  <button type=\"button\" class=\"removelandmark-button btn-xs btn-danger\">X</button>\n</div></p>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    var stack1;

  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.landmarks : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "");
},"useData":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "<p><div class=\"list-item\" data-name=\""
    ...
    + "\">\n  <div>\n    "
    + alias2(alias1(depth0, depth0))
    + "\n  </div>\n  <button type=\"button\" class=\"removerestaurant-button btn-xs btn-danger\">X</button>\n</div></p>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    var stack1;

  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.restaurants : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "");
},"useData":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "<p><div class=\"list-item\" data-name=\""
    ...
    + "\">\n  <div>\n    "
    + alias2(alias1(depth0, depth0))
    + "\n  </div>\n  <button type=\"button\" class=\"removecomment-button btn-xs btn-danger\">X</button>\n</div></p>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    var stack1;
...
var onGetLocations = function onGetLocations(event) {
  event.preventDefault();
  api.getLocations().then(ui.getLocationsSuccess).catch(ui.failure);
};

var onAddLocation = function onAddLocation(event) {
  event.preventDefault();
  var name = $(event.delegateTarget).children('p').text();
  var index = -1;
  if (store.locations) {
    index = store.locations.findIndex(function (loc) {
      return loc.name === name;
    });
  }
  if (index > -1) {
    var id = store.locations[index].id;
    api.getOneLocation(id).then(ui.getOneLocationSuccess).catch(ui.failure);
  } else {
    if (store.user) {
      api.addLocation(name).then(ui.addLocationSuccess).catch(ui.failure);
    } else {}
  }
};

var onAddActivity = function onAddActivity(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  if (store.location) {
    api.addActivity(data.activity.name).then(ui.addActivitySuccess).then(function () {
      return ui.storeOneActivity(data.activity.name);
    }).catch(ui.failure);
  } else {}
};

var onAddLandmark = function onAddLandmark(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  if (store.location) {
    api.addLandmark(data.landmark.name).then(ui.addLandmarkSuccess).then(function () {
      return ui.storeOneLandmark(data.landmark.name);
    }).catch(ui.failure);
  } else {}
};

var onAddRestaurant = function onAddRestaurant(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  if (store.location) {
    api.addRestaurant(data.restaurant.name).then(ui.addRestaurantSuccess).then(function () {
      return ui.storeOneRestaurant(data.restaurant.name);
    }).catch(ui.failure);
  } else {}
};
...
var onAddComment = function onAddComment(event) {
  ...
  var data = getFormFields(event.target);
  if (store.location) {
    api.addComment(data.comment.name).then(ui.addCommentSuccess).then(function () {
      return ui.storeOneComment(data.comment.name);
    }).catch(ui.failure);
  } else {}
};

var onRemoveLocation = function onRemoveLocation(event) {
  event.preventDefault();
  if (store.location) {
    api.removeLocation().then(ui.removeLocationSuccess).catch(ui.failure);
  } else {}
};
...
var addHandlers = function addHandlers() {
  ...
  $('#-addactivity-modal').on('hidden.bs.modal', function () {
    $('#-addactivity-modal input').val('');
  });
  $('#-addlandmark-modal').on('hidden.bs.modal', function () {
    $('#-addlandmark-modal input').val('');
  });
  $('#-addrestaurant-modal').on('hidden.bs.modal', function () {
    $('#-addrestaurant-modal input').val('');
  });
  $('#-addcomment-modal').on('hidden.bs.modal', function () {
